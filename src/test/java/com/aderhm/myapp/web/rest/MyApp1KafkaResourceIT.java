package com.aderhm.myapp.web.rest;

import com.aderhm.myapp.config.KafkaProperties;
...
class MyApp1KafkaResourceIT {
    ...
    void setup() {
        ...
        MyApp1KafkaResource kafkaResource = new MyApp1KafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/my-app-1-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/my-app-1-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
